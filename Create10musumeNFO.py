...
def create_nfo(movie_json):
    ...
    # 3. Extra-fanart
    # latest one
    # https://www.10musume.com/dyn/dla/json/movie_gallery/051421_01.json
    movie_gallery_url = f'https://www.10musume.com/dyn/dla/json/movie_gallery/{movie_id}.json'
    ...
    if movie_gallery_response.status_code == requests.codes.ok:
        ...
        movie_gallery_base_url = f'https://www.10musume.com/dyn/dla/images/'
    ...
    # older one
    # https://www.10musume.com/dyn/phpauto/movie_galleries/movie_id/062519_01.json
    movie_gallery_url = f'https://www.10musume.com/dyn/phpauto/movie_galleries/movie_id/{movie_id}.json'
    movie_gallery_response = requests.get(movie_gallery_url)
    if movie_gallery_response.status_code == requests.codes.ok:
        print(f'movie gallery URL: {movie_gallery_url}')
        movie_gallery_base_url = f'https://www.10musume.com/assets/'
        movie_gallery_json = movie_gallery_response.json()
        images = movie_gallery_json['Rows']
        print(f'total images: ', len(images))
        ...
        for image in images:
            ...
            if image['Protected']:
                image_concat_url = f'{movie_gallery_base_url}member/{movie_id}/{image_url}'
            else:
                image_concat_url = f'{movie_gallery_base_url}sample/{movie_id}/{image_url}'
            image_response = requests.get(image_concat_url)
            extrafanart_file = open(extrafanart_pathname, 'wb')
            extrafanart_file.write(image_response.content)
            extrafanart_file.close()
    ...
    if sample_file_url:
        ...
        trailer = f'{movie_folder_name} - trailer.mp4'
...
if __name__ == '__main__':
    ...
    # https://www.10musume.com/dyn/phpauto/movie_details/movie_id/051421_01.json
    url: str = f'https://www.10musume.com/dyn/phpauto/movie_details/movie_id/{code}.json'
    print(f'url: {url}')
